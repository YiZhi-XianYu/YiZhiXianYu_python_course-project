...
// === 🏆 胜利结算 (播放 Laboon 版) ===
function finishGame() {
    // ✨✨✨ 核心修改：立刻切换到 Laboon (Menu 音乐) ✨✨✨
    // 这会让激昂的战斗曲淡出，温柔的 Laboon 淡入
    switchMusic('MENU');
    ...
    const restartHandler = () => {
        if (gameState === "VICTORY" && opacity > 0.8) {
            ...
            backToMenu();
            // 注意：backToMenu 里面也会调用 switchMusic('MENU')
            // 但因为已经是 MENU 音乐在播放了，代码会自动忽略，不会造成冲突，非常完美。
        }
    };
...
// === ✨ NEW: 音频交叉淡变系统 (Crossfade System) ===
let fadeInterval = null;

function switchMusic(targetState) {
    // targetState: 'MENU' (播放Laboon) 或 'GAME' (播放战斗曲)
